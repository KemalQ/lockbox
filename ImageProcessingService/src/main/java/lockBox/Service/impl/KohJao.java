...
public class KohJao {
     ...
     */
    public Boolean extractBitFromBlock(double[][] block) {
        // 1. Применить DCT к блоку 8x8
        double[][] dctCoefficients = imageProcessing.dct(block);

        // 2. Рассчитать разницу между коэффициентами
        double k = Math.abs(dctCoefficients[4][5]) - Math.abs(dctCoefficients[5][4]);

        // 3. Интерпретировать разницу как бит
        if (k >= 25) {
            return true;  // бит = 1
        }
        else if (k <= -25) {
            ...
            return null;  // не можем определить бит
        }
    }
}
